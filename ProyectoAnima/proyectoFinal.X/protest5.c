 ...
 * Author: PJPE
 *
 * Created on 31 de mayo de 2023, 03:56 PM
 */
...
#include <stdio.h>
#include <stdlib.h>
#define dirpot0EEPROM 0x04
#define dirpot1EEPROM 0x05
#define dirpot2EEPROM 0x06
#define dirpot3EEPROM 0x07
...
void writeEEPROM(uint16_t data, uint16_t adress);
uint16_t readEEPROM(uint16_t adress);
...
int main(void) {
    ...
    while(1){
        ...
        if(RB1 == 1 && botonPrevState == 1){
            writeEEPROM(pot0, dirpot0EEPROM);
            //writeEEPROM(pot1, dirpot1EEPROM);
            //writeEEPROM(pot2, dirpot2EEPROM);
            //writeEEPROM(pot3, dirpot3EEPROM);
            botonPrevState = 1;
        }
        
        temp0 = readEEPROM(dirpot0EEPROM);
        temp1 = readEEPROM(dirpot1EEPROM);
        temp2 = readEEPROM(dirpot2EEPROM);
        temp3 = readEEPROM(dirpot3EEPROM);
